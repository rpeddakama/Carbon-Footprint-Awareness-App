...
def ravel_indices(feature_grid_y, feature_grid_x, num_levels, height, width,
  ...
  Returns:
    indices: A 1D int32 tensor containing feature point indices in a flattened
      feature grid.
  """
  num_boxes = tf.shape(input=feature_grid_y)[1]
  batch_size = tf.shape(input=feature_grid_y)[0]
  size_y = tf.shape(input=feature_grid_y)[2]
  size_x = tf.shape(input=feature_grid_x)[2]
...
def pad_to_max_size(features):
  ...
  """
  heights = [tf.shape(input=feature)[1] for feature in features]
  widths = [tf.shape(input=feature)[2] for feature in features]
  max_height = tf.reduce_max(input_tensor=heights)
  max_width = tf.reduce_max(input_tensor=widths)
  ...
  true_feature_shapes = tf.stack([tf.shape(input=feature)[1:3]
...
def _gather_valid_indices(tensor, indices, padding_value=0.0):
  ...
  padded_tensor = tf.concat(
      [
          padding_value *
          tf.ones([1, tf.shape(input=tensor)[-1]], dtype=tensor.dtype), tensor
      ],
      axis=0,
  )
...
def multilevel_roi_align(features, boxes, box_levels, output_size,
  ...
  with tf.compat.v1.name_scope(scope, 'MultiLevelRoIAlign'):
    ...
    max_feature_height = tf.shape(input=features)[2]
    max_feature_width = tf.shape(input=features)[3]
...
def matmul_crop_and_resize(image, boxes, crop_size, extrapolation_value=0.0,
  ...
  with tf.compat.v1.name_scope(scope, 'MatMulCropAndResize'):
    box_levels = tf.zeros(tf.shape(input=boxes)[:2], dtype=tf.int32)
